...
Droid::Droid(const Droid &droid) : Id(droid.Id), Status(new std::string(*droid.Status)), BattleData(new DroidMemory())
{
...
bool Droid::operator()(const std::string *task, size_t exp)
{
    ...
    this->Energy = std::max(0, (int)this->Energy - 10);
    if (this->BattleData->getExp() >= exp) {
        this->setStatus(new std::string(*task + " - Completed!"));
        this->BattleData->setExp(this->BattleData->getExp() + exp * 0.5);
        return (true);
    } else {
